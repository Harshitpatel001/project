...
export const inngest = new Inngest({ id: "fullstack-ems001" });
...
const attendanceReminderCron = inngest.createFunction(
    { id: "attendance-reminder-cron", triggers: [{ cron: "006 * * *" }] },
    //{ cron: "006 * * *" }, 06:00 UTC = 11:30 AM IST
    async ({ step }) => {
        ...
        const activeEmployees = await step.run
            ("get-active-employees", async () => {
                ...
                return employees.map((e) => ({
                    _id: e._id.toString(),
                    firstName: e.firstName, lastName: e.lastName, email: e.
                        email, department: e.department
                }))
            })
            ...
            await step.run("send-reminder-emails", async () => {
                const emailPromises = absentEmployees.map((emp) => {
                    ...
                    sendEmail({
                        ...
                        <br />
                        <p style="font-size: 16px;">Best Regards, </p>
                        <p style="font-size: 16px; "><strong>QuickEMS</strong></p> I
                        </div>`
                    })
                })
            })
...
return {
    totalActive: activeEmployees.length, onLeave: onLeaveIds.length, checkedIn: checkedInIds.length, absent:
