...
const AttendanceStats = ({ history }) => {
    const totalPresent = history.filter((h) => h.status === "PRESENT" | h.status ===
        " LATE").length;
...
bg-slate-500/70 group-hover:bg-indigo-500/70"/>
                    <div className='p-3 bg-slate-100 rounded-lg group-hover:bg-indigo-50
...
group-hover: text-indigo-600 transition-colors duration-200"/>
                    ...
                    <div>
                        ...
                        <p className="text-2x1 font-medium text-slate-900 tracking-tight">
                            {s.value}</p>
                    </div>
                </div>
            ))}
