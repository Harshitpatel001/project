...
const ProfileForm = ({ initialData, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            ...
            setError(err.reponse?.data?.error || err.message);
        }finally{
            setLoading(false)
        }
    }
...
shrink-0" />
            ...
            {message && (
                <div className='bg-emerald-50 text-emerald-700 p-4 rounded-x1 text-sm border border-emerald-200 mb-6 flex items-start gap-3'>
...
text-slate-700 mb-2">Name</label>
                        <input disabled value={`${initialData.firstName} ${initialData.lastName}`} className='bg-slate-50
...
mb-2">Bio</label>
                    ...
                    <p className='text-xs text-slate-400 mt-1.5'>This will be
                        displayed on your profile.</p>
                </div>
                {initialData.isDeleted ? (
                    <div className='pt-2'>
                        <div className='p-4 bg-rose-50 border border-rose-200
rounded-x1 text-center'>
                            <p className='text-rose-600 font-medium
tracking-tight'>Account Deactivated</p>
                            <p className='text-sm text-rose-500 mt-0.5'>You can
                                no longer update your profile.</p>
                        </div>
                    </div>
                ) : (
