...
const PrintPayslip = () => {
  ...
  useEffect(() => {
    api.get(`/payslips/${id}`).then((res)=> setPayslip(res.data))
    .catch(console.error).finally(()=> setLoading(false))
  }, [id])

  if (loading) return <Loading />

  if (!payslip) return <p className='text-center py-12 text-slate-400' >Payslip not
    found</p>
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 bg-white animate-fade-in">
      <div className='text-center border-b border-slate-200 pb-6 mb-8'>
        <h1 className='text-l font-bold text-slate-900 tracking-tight'>PAYSLIP</h1>
        <p className='text-slate-500 text-sm mt-1'>{format(new Date(payslip.year,
          payslip.month - 1), "MMMM yyyy")}</p>
      </div>
...
mb-1'>Position</p>
        ...
        <div>
          ...
          <p className='font-semibold text-slate-900'>{format(new Date(payslip.year,
            payslip.month - 1), "MMMM yyyy")}</p>
        </div>
      </div>
...
tracking-wider'>Amount</th>
          ...
          <tbody>
            <tr className="border-t border-slate-100">
              ...
              <td className='text-right py-3 px-4 text-slate-900 font-medium'>${payslip.
                basicSalary?.toLocaleString()}</td>
            </tr>
            <tr className="border-t border-slate-100">
              <td className='py-3 px-4 text-slate-700'>Allowances</td>
              <td className='text-right py-3 px-4 text-slate-900 font-medium'>+${payslip.
                allowances?.toLocaleString()}</td>
            </tr>
            <tr className="border-t border-slate-100">
              <td className='py-3 px-4 text-slate-700'>Deductions</td>
              <td className='text-right py-3 px-4 text-slate-900 font-medium'>-${payslip.
                deductions?.toLocaleString()}</td>
            </tr>
            <tr className="border-t-2 border-slate-200 bg-slate-50">
              ...
              <td className='text-right py-4 px-4 font-bold text-slate-900 text-lg'>$
                {payslip.netSalary?.toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
